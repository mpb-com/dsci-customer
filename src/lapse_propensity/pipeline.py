...
def pipe(bq, calibration_window_days=None, use_calibration: bool = True):
    """
    ...
    log.info("=" * 80)
    log.info(f"Starting lapse propensity model ({mode})")
    log.info("=" * 80)
    ...
    if use_calibration:
        ...
        log.info(
            f"Calibration window:   {calibration_start.strftime('%Y-%m-%d')} → {calibration_end.strftime('%Y-%m-%d')}"
        )
        ...
        all_features = create_btyd_features(transactions)
        log.info(f"Training model on {len(all_features)} customers")

        prod_model = create_model_from_config()
        prod_model.fit(all_features)

        # Get raw predictions
        all_features["p_alive"] = prod_model.p_alive(all_features).round(4)
    ...
    all_features["customer_status"] = all_features["p_alive"].apply(
